...
def pivot_months_pandas(data):
    ...
    """
    # totals = pd.read_csv('precipitation.csv', parse_dates=[2]).set_index(keys=['name'])
    totals = data.drop(axis=1, labels= {"station", "latitude", "longitude", "elevation"})
    totals['month'] = totals['date'].dt.strftime("%Y-%m")
    totals = totals.drop(axis=1, labels= "date")
    totals = totals.groupby(['name', 'month']).sum().reset_index()
    totals = totals.pivot(index='name', columns='month', values='precipitation')
    # totals.to_csv('totals.csv')

    # counts = pd.read_csv('precipitation.csv', parse_dates=[2]).set_index(keys=['name'])
    counts = data.drop(axis=1, labels= {"station", "latitude", "longitude", "elevation", "precipitation"})
    counts['month'] = counts['date'].dt.strftime('%Y-%m')
    counts = counts.drop(axis=1, labels= "date")
    counts = counts.groupby(['name', 'month']).size().reset_index()
    counts.columns = ['name', 'month', 'counts']
    ...
    # Making totals.csv
    totals = pd.read_csv('precipitation.csv', parse_dates=[2]).set_index(keys=['name'])
    totals = totals.drop(axis=1, labels= {"station", "latitude", "longitude", "elevation"})
    totals['month'] = totals['date'].dt.strftime("%Y-%m")
    totals = totals.drop(axis=1, labels= "date")
    totals = totals.groupby(['name', 'month']).sum().reset_index()
    totals = totals.pivot(index='name', columns='month', values='precipitation')
    ...
    # Making counts.csv
    counts = pd.read_csv('precipitation.csv', parse_dates=[2]).set_index(keys=['name'])
    counts = counts.drop(axis=1, labels= {"station", "latitude", "longitude", "elevation", "precipitation"})
    counts['month'] = counts['date'].dt.strftime('%Y-%m')
    counts = counts.drop(axis=1, labels= "date")
    counts = counts.groupby(['name', 'month']).size().reset_index()
    counts.columns = ['name', 'month', 'counts']
